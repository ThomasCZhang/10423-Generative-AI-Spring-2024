...
\newcommand{\outDate}{Feb. 07, 2024}
\newcommand{\dueDate}{Feb. 19, 2024}
\newcommand{\taNames}{Ifigeneia, Qin, Jing, Jacob}
...
\part Suppose we want to define a GAN-inspired model for inpainting grayscale images. Each original image is a matrix $\xv \in (0,1)^{N_w \times N_h}$ with scalars between $0$ and $1$ exclusive. The image is accompanied by a binary pixel mask $\mv \in \{0,1\}^{N_w \times N_h}$, where $m_{ij}=1$ indicates that the pixel should be masked out and $m_{ij}=0$ indicates the pixel should be left intact. Our model is a variant of U-Net that takes in an input image $\xv$ and and a mask $\mv$ and returns a reconstructed image $\xv'$ with the masked pixels filled in by the model, $\xv' = g_{\theta}(\xv, \mv)$. 
You decide to train your model by using two loss functions in combination.

\begin{subparts}

\subpart[2] \textbf{Short answer:} Define a squared error loss $\ell_{mse}(\theta)$ for one example $(\xv, \mv)$ that, when minimized, encourages the masked out pixels of the original image to match the reconstructed pixels output by the model. To be safe, your definition must ensure that the masked out pixels are never visible to $g_{\theta}$.
    \begin{answer_box}[title=,height=2cm, width=14cm]
    \end{answer_box}
    
\subpart[2] \textbf{Short answer:} Now suppose we have a discriminator $d_{\phi}(\xv)$ that returns the probability that $\xv$ is a real image and not from the generator $g_{\theta}$. How could your objective function above be combined with GAN-style training to train the generator so that it is effective at inpainting, and the discriminator so that it is effective at distinguishing real from inpainted images? Briefly describe a GAN-style objective $\ell_{gan}(\phi, \theta)$ for one example $(\xv, \mv)$ and your training algorithm.
...
\begin{equation}
   \log p(\xv) \ge \mathbb{E}_{q(\xv_1 \mid \xv_0)}[\log p_\theta (\xv_0 \mid \xv_1) ] + \sum_{t=1}^{T-1} \underbrace{\mathbb{E}_{q(\xv_{t-1}, \xv_t, \xv_{t+1} \mid \xv_0)}\left[ \log \frac{p_\theta(\xv_t \mid \xv_{t+1})}{q(\xv_t \mid  \xv_{t-1})}\right]}_{\mathcal{L}_t}+ C,
...
\clearpage
\part [2] Can you explain what is the effect of the term ${\mathcal{L}_t}$ on the reverse process $p_\theta(\xv_t \mid \xv_{t+1})$ of the diffusion model when we try to maximize the ELBO in Equation~\eqref{eq:elbo_breakdown} and why? When is this term minimized? %If we were omitting this term from the objective function (we were maximizing only the first term of Equation~\ref{eq:elbo_breakdown}), how would the quality of novel images generated by the model be impacted?
...
\uplevel{\subsection*{Image Denoising}}\label{sec:denoising}
It can be proved (see Appendix A by \href{https://arxiv.org/pdf/2006.11239.pdf}{Ho et al., 2020}) that Equation~\ref{eq:elbo_breakdown} can be rewritten:
\begin{equation}
...
\part [3] 
The terms $q(\xv_{t-1} \mid \xv_{t}, \xv_{0})$ in Equation~\ref{eq:elbo_denoising} can act as a ground-truth signal, since it defines how to denoise a noisy image $\xv_t$
with access to what the final, completely denoised image $\xv_0$ should be.
...
\part [1] 
The term $\mathcal{L}'_t$ in Equation~\ref{eq:elbo_denoising} is a denoising matching term. When we train $\theta$ to maximize the ELBO of Equation~\ref{eq:elbo_denoising}, can you explain what is the effect of term  $\mathcal{L}'_t$ on the denoising step $p_\theta(\xv_{t-1} \mid \xv_{t})$?

\begin{answer_box}[title=,height=4cm, width=15cm]
\end{answer_box}

% \part[2]
% Using the reparameterization trick (Equation~\ref{eq:reparametrization_trick}), describe qualitatively (no mathematical derivations are required), 

%\part [5] 

% It can be proved that $q(\xv_{t-1} \mid \xv_{t}, \xv_{0})$ is Gaussian: 
% \begin{equation}
% q(\xv_{t-1} \mid \xv_{t}, \xv_{0}) = \mathcal{N}(\xv_{t-1}; \tilde{\boldsymbol{\mu}_q}, \tilde{\sigma}_q)
% \end{equation}

% Let The terms $\matcal{L'}_t$ are also called .
...
% \part[0] TODO: This question should be all about the mechanics of the algorithm. The hope is that if they can "implement" diffusion on paper, then they can translate it more easily to code.
...
%you will explore the implementation and assess its impact on a small dataset. By understanding the mechanisms behind diffusion processes and their application in probabilistic modeling, 
%DDPM is at the forefront of probabilistic modeling, utilizing diffusion processes to generate high-quality samples and effectively tackle denoising tasks.
...
\uplevel{\subsection*{Diffusion}}
    ...
    % \textbf{Probabilistic Model:} TODO
    ...
    % The above equation depends on \( \xv_{t-1} \) for each \( \xv_t \) which implies that to get the noised output at time \( t \), we need the noised output at \( t - 1 \). However, we could use a reparameterization trick to generate any arbitrary noised input \( \xv_t \) directly, just given our initial image \( \xv_0 \):
    % \begin{equation}
    %     \begin{aligned}
    %         q(\xv_t | \xv_0) = \mathcal{N}(\xv_t; \sqrt{\bar{\alpha_t}}\xv_0, (1 - \alpha_t)I), \text{ where } \bar{\alpha_t} = \prod_{i=1}^{t} \alpha_i.
    %     \end{aligned}
    % \end{equation}\label{eq:hw2_forward}
    % Therefore, we could sample from \( q(\xv_t | \xv_0) \) to produce
    \begin{equation}
        \xv_t = \sqrt{\bar{\alpha_t}}\xv_0 + \sqrt{1 - \alpha_t} \epsilonv, \text{where } \epsilonv \sim \mathcal{N}(0,I).
        \label{eq:xt}
    \end{equation}
    %where $\epsilonv \sim \mathcal{N}(0,I)$.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\label{eq:encoder_3}
\end{equation}
    %a denoising network $\epsilon_\theta$ is employed, which utilizes the noised image and its timestamp to predict the noise $\epsilonv_t = \epsilon_\theta(\xv_t, t)$ introduced during the forward process.
    \par
...
\end{equation}
    ...
    % Also, by reversing Eq.~\ref{eq:hw2_forward}, we could represent estimated $\hat x_0$ from $x_t$ and predicted noise $\epsilon_t$:
    % \begin{equation}
    %     \hat \xv_0 = \frac{1}{\sqrt{\bar{\alpha}_t}} \left( \xv_t - \sqrt{1 - \bar{\alpha}_t} \epsilonv_t \right).
    % \end{equation}
    %  By iteratively applying this process:
    % \begin{equation}
    %     \xv_{t-1} = \tilde{\muv}_t(\xv_t, \hat{\xv}_0)+\sigma_t \zv, \text{where } \zv \sim \mathcal{N}(\boldsymbol{0},\boldsymbol{I})
    % \end{equation}
    %  starting from $\xv_T \sim \mathcal{N}(\boldsymbol{0},\boldsymbol{I})$, we can recreate the original sample from a Gaussian noise input, , through the \textit{reverse diffusion process}.

    % \textbf{Reverse Process (Noise $\rightarrow$ Image):} From the paper, we know that the reverse conditional probability is tractable when conditioned on \( x_0 \):
    % \begin{equation}
    %     \begin{aligned}
    %         q(x_{t-1} | x_t, x_0) = \mathcal{N}(x_{t-1}; \tilde\mu_t(x_t, x_0), \tilde{\beta}_t I), \\
    %         \text{where } \tilde{\mu}_t = \sqrt{\frac{\alpha_t(1 - \alpha_{t-1})}{1 - \alpha_t}} x_t + \sqrt{\frac{\alpha_{t-1}\beta_t}{1 - \alpha_t}} x_0, \\
    %         \sigma^2_t = \tilde{\beta}_t = \frac{1 - \alpha_{t-1}}{1 - \alpha_t} \beta_t.
    %     \end{aligned}
    % \end{equation}
    % This formulation allows us to recreate the original sample from a Gaussian noise input, $x_T \sim \mathcal{N}(0,1)$, through the \textit{reverse diffusion process}.
    % \par
    % To estimate $x_0$, a denoising network $\epsilon_\theta$ is employed, which utilizes the noised image and its timestamp to predict the noise $\epsilon_t = \epsilon_\theta(x_t, t)$ introduced during the forward process.
    % % It is noteworthy that we could estimate $x_0$ by inverting the 
    % % To predict $x_0$, we use a denoising network, which takes the noised image and timestamp as inputs to predict the noise $\epsilon_t = f(x_t, t)$ which was added on in the forward process. 
    % By reversing Eq.~\ref{eq:hw2_forward}:
    % \begin{equation}
    %     \hat{x}_0 = \frac{1}{\sqrt{\bar{\alpha}_t}} \left( x_t - \sqrt{1 - \bar{\alpha}_t} \epsilon_t \right),
    % \end{equation}
    % we could deduce estimated $\hat x_0$ from $x_t$ and predicted noise $\epsilon_t$.
    % Consequently, $x_{t-1}$ can be obtained as $\tilde{\mu}_t(x_t, \hat{x}_0)+\sigma_t$ and by iteratively applying this process, we can recover the image.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\part[4] \textbf{Training:} Plot the training loss of your Diffusion model above over 1,000 training steps with the recommended parameters in the above command line. Your model should be generating blurry cats at this point. Additional recommended flags: \lstinline{--train_steps 1000}.
...
During training time, the starter code uses the `compute\_fid' function from `clean-fid' to compute the FID value between training samples and generated samples. Get the FID value every 100 training steps and plot it over 1,000 training steps with the recommended parameters in the above command line. Additional recommended flags: \lstinline{--save_and_sample_every 100 --train_steps 1000 --fid}.
...
Use the trained model after 1,000 steps to illustrate the forward diffusion process on the initial batch of the training dataset at key time intervals: 0\%, 25\%, 50\%, 75\%, and 99\% of the total timesteps. The resulting figure should resemble the provided sample, though the images will vary due to inherent randomness.
...
\part[5] \textbf{Visualization:} Finally, train the model for a full 10,000 iterations and show the images generated in the last sample batch. The images should be a substantial improvement over training with fewer iterations. Additional recommended flags: \lstinline{--save_and_sample_every 1000 --train_steps 10000 --fid False}.

[Expected runtime on Colab T4: 1-3 hours]
