...
\newcommand{\outDate}{Jan. 18, 2024}
\newcommand{\dueDate}{Jan. 24, 2024}
\newcommand{\taNames}{Haoyang, Jing, Qin, Ifigeneia, and Tiancheng}
...
%% To HIDE SOLUTIONS (to post at the website for students), set this value to 0: \providecommand{\issoln}{0}
%\providecommand{\issoln}{0}
%\providecommand{\issoln}{1}
...
This assignment is \emph{primarily} a reading assignment. You will read both the starter code and various tutorials.
...
\part[3] Run the image classifier with the default hyperparameters. Using \lstinline{wandb}, plot the training loss of each batch vs. the number of examples seen so far by the optimizer. The loss should be the average loss, taking the average within each batch.
...
\part[2] Next consider what happens if we work with really tiny images. Resize each image to $(28 \times 28)$ (the standard size of an MNIST digit). What is the test accuracy in this case?  (Revert back to resizing images to $(256 \times 256)$ after this question.)
...
\part Our current optimizer is stochastic gradient descent (\lstinline{torch.optim.SGD}). Change to some other optimizer from \lstinline{torch.optim}\footnote{\url{https://pytorch.org/docs/stable/optim.html}} and adjust the hyperparameters to see if you can achieve better performance than SGD. (If you don't get better performance, you can still receive full credit here.) 

\begin{subparts}

    \subpart[2] Which optimizer did you choose and what hyperparameters did you select?

        \begin{answer_box}[title=,height=5cm, width=15cm]
        \end{answer_box}
        
\newpage
    \subpart[3] Then plot the train accuracy and validation accuracy on the same plot with the number of epochs on the horizontal axis. Include a run with SGD and with the new optimizer. 
...
\part The model we're using is incredibly simple: a feed-forward neural network with two hidden layers of size 512 units, and ReLU activations. Define a new model that is different in some interesting way. You should review the documentation of \lstinline{torch.nn}\footnote{\url{https://pytorch.org/docs/stable/nn.html}} and select some different modules and put them together in some interesting way.
The only requirement is that you do \emph{not} use \lstinline{nn.Sequential}---that is, we would like you to make the order of computation explicit in your call to forward. (If you don't get better performance, you can still receive full credit here.) 
...
\begin{figure}[b]  
    ...
    \label{fig:llmdist}    
    \includegraphics[width=0.5\linewidth]{fig/llm_dist.png}
\end{figure}
...
\part[3] Run the code three times and report the validation and test accuracies. (You can easily create such a table on the ``Runs'' tab of \lstinline{wandb} if these are logged.) 
    ...
    \begin{answer_box}[title=,height=5cm, width=10cm]
    \begin{center}
    \begin{tabular}{ccc}
        \toprule
        & Val. Acc. & Test Acc. \\
...
\part Carefully read the documentation for \lstinline{nn.Embedding} and then \lstinline{nn.EmbeddingBag} paying close attention to the \lstinline{padding_idx} parameter the associated Examples. Next read the code for \lstinline{PadSequence} so that you understand what it is doing. 
...
\end{subparts}
\uplevel{(Revert any changes you made for the previous question.)}


\part Replace the simple \lstinline{TextClassificationModel} with an LSTM-based classifier. (You are welcome to use any of the code presented in the HW0 recitation.)
...
\begin{subparts}
    ...
    \subpart[4] Run the code three times and report the validation and test accuracies.
...
\newpage
\part[2] Using the original \lstinline{TextClassificationModel}, switch the optimizer to Adam, then run the code three times and report the validation and test accuracies. 
